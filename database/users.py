...
class UsersDB:
    ...
    def log_login(self, username, display_name, email, groups, is_admin, ip=None, user_agent=None):
        ...
        with self.db.get_connection() as conn:
            ...
            # Convert groups list to comma-separated string (Handles empty lists too)
    ...
    def get_all_user_preferences(self, username):
        ...
        with self.db.get_connection() as conn:
            ...
            preferences = {row['preference_key']: row['preference_value'] for row in results}
            return preferences
            ...
            query = """
                WITH UserInfo AS (
                    SELECT
                        ...
                        login_date,
                        ip_address,
                        ROW_NUMBER() OVER(PARTITION BY username ORDER BY login_date DESC) as rn
                    FROM UserLogins
                ), LoginCounts AS (
                    SELECT
                        username,
                ...
                SELECT
                    ...
                    lc.first_login,
                    lc.last_login,
                    ui.ip_address as last_ip,
                    CASE
                        WHEN ui.is_admin = 1 THEN 'Admin'
                        ELSE 'User'
                    END as access_level
                FROM UserInfo ui
                JOIN LoginCounts lc ON ui.username = lc.username
                WHERE ui.rn = 1
    ...
    def get_login_statistics(self):
        ...
        with self.db.get_connection() as conn:
            ...
            query_total = "SELECT COUNT(DISTINCT username) as total FROM UserLogins"
            result = conn.execute_query(query_total)
            ...
            """
            result = conn.execute_query(query_today)
            ...
            """
            result = conn.execute_query(query_week)
            ...
            """
            stats['daily_trends'] = conn.execute_query(query_trends)
    ...
    def get_user_details(self, username):
        ...
        with self.db.get_connection() as conn:
            ...
            stats_query = """
                SELECT
                    COUNT(*) as total_logins, MIN(login_date) as first_login,
                    MAX(login_date) as last_login,
            ...
            if conn.check_table_exists('AuditLog'):
                audit_query = """
                    ...
                    FROM AuditLog WHERE changed_by = ?
                """
                audit_stats = conn.execute_query(audit_query, (username,))
                if audit_stats: user['audit_stats'] = audit_stats[0]
            ...
            if conn.check_table_exists('Downtimes'):
                downtime_query = """
                    ...
                    FROM Downtimes WHERE entered_by = ? AND is_deleted = 0
                """
                downtime_stats = conn.execute_query(downtime_query, (username,))
                if downtime_stats: user['downtime_stats'] = downtime_stats[0]
