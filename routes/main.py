...
ADDED: Username stripping logic for email format login.
"""
...
def login():
    if request.method == 'POST':
        ...
        # --- ADDED: Username stripping logic ---
        username_to_auth = username_input
        domain_part = '@wepackitall.com' # Or dynamically use Config.AD_DOMAIN if needed
        if username_input.lower().endswith(domain_part):
            username_to_auth = username_input[:-len(domain_part)]
            print(f"ℹ️ Stripped domain from login input: '{username_input}' -> '{username_to_auth}'")
        # --- END ADDED ---

        if username_to_auth and password: # Use the potentially stripped username
            user_info = authenticate_user(username_to_auth, password) # Pass stripped username
            if user_info:
                # Check for existing active session
                existing_session = sessions_db.get_active_session(user_info['username']) # Use actual username from auth result
                ...
                sessions_db.create_session(new_session_id, user_info['username'], ip, user_agent) # Use actual username
                ...
                print(f"✅ User {user_info['username']} logged in successfully (session: {new_session_id[:8]}...)") # Use actual username
                ...
                if request.form.get('force_login'):
                    ...
                    print(f"❌ Login failed for user input: {username_input} (attempted as: {username_to_auth})") # Log both input and attempted
        else:
            if request.form.get('force_login'):
        ...
        if status_info['db_connected']:
            with db.get_connection() as conn:
                ...
                if conn.check_table_exists('UserLogins'):
                     result = conn.execute_query("SELECT COUNT(DISTINCT username) as count FROM UserLogins WHERE login_date >= CAST(GETDATE() AS DATE)")
                     status_info['users_today'] = result[0]['count'] if result else 0
